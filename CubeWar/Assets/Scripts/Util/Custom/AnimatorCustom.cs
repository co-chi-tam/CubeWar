...
namespace CubeWar {
	...
	public class AnimatorCustom : MonoBehaviour, IAnimation {
		...
		public void RandomAnimationFrame() {
			StartCoroutine (HandleRandomAnimationFrame ());
		}

		private IEnumerator HandleRandomAnimationFrame() {
			this.SetSpeed (UnityEngine.Random.Range (0, 999));
			yield return WaitHelper.WaitEndOfFrame;
			this.SetSpeed (1f);
		}
